...
class svmSet:
    ...
    Parameters
    ----------
    SVM : sklearn.svm.SVC, sklearn.svm.SVR, or sklearn.svm.OneClassSVM
        Prototype estimator copied once per cross-validation split.
    cvSet : mistic.cvSet.cvSet
        Dataset and reusable training/validation splits.
    ...
    performance_ : mistic.utility.dotdict or list of dotdict
        Aggregate or member-level validation metrics.
    feature_rank : numpy.ndarray
        Feature ranks produced during feature selection.
    unified_model_ : sklearn estimator or None
        Final estimator trained on the unified feature subset.
    unified_prediction_features_ : numpy.ndarray or None
        Features used by ``unified_model_``.
    decision_value_cutoff_ : float
    ...
    def _is_one_class(self):
        """Return whether the configured estimator is a OneClassSVM.

        Returns
        -------
        bool
            True when the estimator prototype is a ``OneClassSVM``.
        """
    ...
    def __init__(
        ...
        Parameters are documented on :class:`svmSet`.
    ...
    def __setstate__(self, state):
        ...
        ----------
        state : dict
            Attribute mapping loaded from a pickle.
    ...
    def _update_unified_feature_attributes(self):
        ...
        -------
        None
            Updates ``unified_features``, ``unified_feature_rank``, and
            ``unified_sorted_features``.
        """
        if self.separate_feature_sets:
            feature_sets = [np.asarray(features, dtype=int) for features in self.features]
        ...
        if self.separate_feature_sets:
            ...
            self.unified_feature_rank = np.asarray(self.feature_rank, dtype=float).copy()
        self.unified_sorted_features = np.argsort(self.unified_feature_rank, kind="stable")

    def _train_models(self):
        """Fit every member estimator using its current kernel matrix.

        Returns
        -------
        None
            Updates the fitted estimators in :attr:`models`.
        """
        for i in range(self.num_models):
    ...
    def _update_kernel_matrix(self):
        """Recompute member or shared kernels for the current configuration.

        Returns
        -------
        None
            Replaces the cached ``kernel_matrix_`` contents.
        """
        if self.separate_feature_sets | self.separate_parameters:
            for i in range(self.num_models):
                if self.separate_feature_sets:
    ...
    def _reset_kernel_matrix(self):
        """Allocate empty kernel-matrix storage for the ensemble mode.

        Returns
        -------
        None
            Replaces ``kernel_matrix_`` with correctly shaped zero arrays.
        """
        if self.separate_feature_sets | self.separate_parameters:
            self.kernel_matrix_ = []
            for i in range(self.num_models):
                self.kernel_matrix_.append(np.zeros((self.num_samples, len(self.cv.train[i]))))
        else:
    ...
    def _get_kernel_matrix(self, indices_1, indices_2, model_index=None):
        """Extract a kernel submatrix for specified sample indices.

        Parameters
        ----------
        indices_1, indices_2 : array-like of int
            Row and training-column sample indices, respectively.
        model_index : int or None, default=None
            Member index required when kernels are stored per model.

        Returns
        -------
        numpy.ndarray
            Requested precomputed-kernel submatrix.
        """
        if isinstance(self.kernel_matrix_, list):
            ...
            column_indices = np.fromiter(
                (positions[sample] for sample in indices_2), dtype=int, count=len(indices_2)
            )
    ...
    def _score_models(self):
        """Score fitted members and store per-member or aggregate results.

        Returns
        -------
        None
            Stores metrics and selected parameters in ``performance_``.
        """
        accuracy = []
        for i in range(self.num_models):
            ...
            if self.separate_parameters:
                ...
                if i == 0:
                    ...
                    accuracy = {key: accuracy[key] + score[key] for key in score}
        ...
        if self.separate_parameters:
            ...
            self.performance_ = dotdict({key: accuracy[key] / self.num_models for key in accuracy})
    ...
    def mean_performance(self):
        ...
        if (
            ...
            raise TypeError("performance_ must be a mapping or a non-empty sequence of mappings")
        ...
        for key in rows[0]:
            ...
            if all(array.ndim == 0 and np.issubdtype(array.dtype, np.number) for array in arrays):
    ...
    def tune_models(self, parameter_grid):
        ...
        parameter_grid : iterable of mistic.utility.paramSet
            Candidate estimator and kernel parameter combinations.

        Returns
        -------
        None
            Stores the winning members, parameters, performance, and unified
        ...
        tune_performance = {}
        for result, parameter_set in enumerate(parameter_grid):
        ...
        if (
            not isinstance(self.SVM, SVR)
            and not self._is_one_class()
            and np.asarray(self.models[0].classes_).size == 2
        ):
    ...
    def _tune_member_models(self, parameter_grid):
        ...
        ----------
        parameter_grid : iterable of mistic.utility.paramSet
            Parameter candidates forwarded to :meth:`tune_models`.
    ...
    def _ranked_unified_prediction_features(self):
        ...
        -------
        numpy.ndarray
            Ranked active features, limited by ``knee_num_features_``.
        """
        ...
        if ranking is None:
            ...
            ranked = np.asarray([feature for feature in ranking if feature in unified], dtype=int)
    ...
    def _score_unified_fold(self, model, kernel_matrix, y_true):
        ...
        model : sklearn estimator
            Fitted candidate unified estimator.
        kernel_matrix : numpy.ndarray
            Validation-to-training precomputed kernel.
        y_true : numpy.ndarray
            Validation labels or regression targets.

        Returns
        -------
        float
            Aggregate validation score appropriate to the estimator type.
        """
        ...
        return (1 - calibration_weight) * discrimination + calibration_weight * calibration
    ...
    def fit_unified_model(self, parameter_grid):
        ...
        for parameters in parameter_grid:
            ...
            for train_indices, test_indices in zip(self.cv.train, self.cv.test):
                ...
                test_kernel = self.kernel.compute(
                    self.cv.X[test_indices],
                    feature_index=features,
                    parameters=parameters.kernel,
                    Y=self.cv.X[train_indices],
                )
                fold_scores.append(
                    self._score_unified_fold(model, test_kernel, self.cv.y[test_indices])
                )
        ...
        training_indices = (
            np.flatnonzero(self.cv.y == 1)
            if self._is_one_class() and getattr(self.cv, "type", None) == "one-class"
    ...
    def _optimal_f1_cutoff(decision_values, y_true, positive_class):
        ...
        decision_values : array-like
            Continuous binary-classification decision values.
        y_true : array-like
            Observed class labels corresponding to ``decision_values``.
        ...
        -------
        float
            F1-maximizing cutoff, with ties resolved toward zero.
        """
        ...
        for index, cutoff in enumerate(candidates):
            ...
            scores[index] = 2 * true_positive / denominator if denominator else 0.0
    ...
    def calibrate_decision_value_cutoff(self):
        ...
        if classes.size != 2:
            raise ValueError("decision-value cutoff calibration requires binary classification")

        calibration_indices = getattr(self.cv, "development_indices_", np.arange(len(self.cv.y)))
    ...
    def _get_support_vectors(self, model_index):
        """Return original-space support vectors for one member.

        Parameters
        ----------
        model_index : int
            Index of the fitted ensemble member.

        Returns
        -------
        numpy.ndarray
            Original input rows corresponding to the member's support vectors.
        """
    ...
    def _inference_kernel(self, X, model_index):
        ...
        X : numpy.ndarray of shape (n_samples, n_features)
            New observations.
        model_index : int
            Member whose feature set, parameters, and support vectors are used.
        ...
        support_kernel = self.kernel.compute(
            X, feature_index=feature_index, parameters=parameters, Y=self.cv.X[support_indices, :]
        )
        ...
        kernel_matrix = np.zeros((len(X), len(self.X_ind[model_index])), dtype=support_kernel.dtype)
        kernel_matrix[:, support_positions] = support_kernel
        return kernel_matrix

    def _unified_inference_kernel(self, X):
        """Build a sparse-width kernel for unified-model inference.
        ...
        -------
        numpy.ndarray
            Precomputed kernel with the unified training-set width.
        """
        ...
        support_kernel = self.kernel.compute(
            X,
            feature_index=self.unified_prediction_features_,
        ...
        kernel_matrix = np.zeros(
            (len(X), len(self.unified_training_indices_)), dtype=support_kernel.dtype
        )
        kernel_matrix[:, support_positions] = support_kernel
        return kernel_matrix
    ...
    def _freeze_parameter(value):
        ...
        ----------
        value : object
            Nested mapping, sequence, array, or scalar parameter value.

        Returns
        -------
        hashable
            Stable recursively frozen representation of ``value``.
        """
        if isinstance(value, dict):
            return tuple(
                sorted((key, svmSet._freeze_parameter(item)) for key, item in value.items())
            )
    ...
    def _kernel_configuration(self, parameter_set):
        """Return a comparable signature for parameters and features.

        Parameters
        ----------
        parameter_set : mistic.utility.paramSet or list of paramSet
            Kernel parameters to combine with the active features.

        Returns
        -------
        tuple
            Hashable kernel-parameter and feature signature.
        """
        if isinstance(parameter_set, list):
    ...
    def _update_parameters(self, parameter_set, update_kernel=True):
        ...
        ----------
        parameter_set : mistic.utility.paramSet or list of paramSet
            Parameters applied globally or per member.
        update_kernel : bool, default=True
            Whether to rebuild a stale cached kernel.
        ...
        if isinstance(parameter_set, list):
            for i in range(self.num_models):
                for model_param in parameter_set[i].model:
                    setattr(self.models[i], model_param, parameter_set[i].model[model_param])
        else:
            for i in range(self.num_models):
                for model_param in parameter_set.model:
    ...
    def _remove_features(self, to_remove, model_index=None, update_kernel=True):
        """Remove perturbation groups globally or from one member.

        Parameters
        ----------
        to_remove : array-like of int
            Features whose complete perturbation groups are removed.
        model_index : int or None, default=None
            Member to modify, or ``None`` for the shared feature set.
        update_kernel : bool, default=True
            Whether to rebuild the cached kernel after removal.
        ...
        current_features = current_features[~np.isin(current_features, expanded_removal)]
        ...
        if model_index is not None:
            ...
            self.removed_features_ = np.append(self.removed_features_, expanded_removal)
        self._update_unified_feature_attributes()
    ...
    def _set_features(self, features, model_index=None, update_kernel=True):
        ...
        features : array-like of int
            Features to retain; perturbation-set ordering is preserved.
        model_index : int or None, default=None
            Member to modify, or ``None`` for the shared feature set.
        update_kernel : bool, default=True
            Whether to rebuild the cached kernel.
    ...
    def _add_features(self, to_add, model_index=None, update_kernel=True):
        ...
        Parameters
        ----------
        to_add : array-like of int
            Features whose complete perturbation groups are activated.
        model_index : int or None, default=None
            Member to modify, or ``None`` for the shared feature set.
        update_kernel : bool, default=True
            Whether to rebuild the cached kernel.
    ...
    # ------------------------------------------------------------------
    # Rank-guided stochastic feature refinement
    # ------------------------------------------------------------------
    def stochastic_feature_selection(
        self,
        parameter_grid,
        n_iterations=100,
        feature_ranker=None,
        ...
        ----------
        parameter_grid : iterable of mistic.utility.paramSet
            Parameter candidates used to retune each proposal.
        n_iterations : int, default=100
            Maximum number of proposed feature moves.
        feature_ranker : callable
            Callable returning group ranks for a model and ranking subset.
        set_for_rank : str, default="train"
            Name of the CV index collection used by ``feature_ranker``.
        temperature : float, default=0.05
            Initial simulated-annealing temperature.
        cooling_rate : float, default=0.97
            Multiplicative temperature decay per iteration.
        add_probability : float, default=0.5
            Probability of proposing additions when both directions are valid.
        random_seed : int or None, default=None
            Seed controlling proposal selection and acceptance.
        update_all_models : bool, default=False
            Whether separate-feature members propose joint moves.
        use_ensemble_validation : bool, default=False
            Score moves on the reserved ensemble holdout.
        expected_changes_per_model : float, default=1.0
            Expected number of groups changed by an eligible model.
        preserve_feature_count : bool, default=False
            Replace groups through equal-sized swaps instead of adding/removing.
        convergence_patience : int or None, default=20
            Non-improving proposals allowed before stopping early.
        convergence_min_delta : float, default=0.0
            Minimum objective gain counted as an improvement.
        ...
        """
        if isinstance(n_iterations, (bool, np.bool_)) or not isinstance(
        ...
        if isinstance(expected_changes_per_model, (bool, np.bool_)) or not isinstance(
            expected_changes_per_model, (int, float, np.integer, np.floating)
        ):
            raise TypeError("expected_changes_per_model must be numeric")
        expected_changes_per_model = float(expected_changes_per_model)
        if not np.isfinite(expected_changes_per_model) or expected_changes_per_model < 1:
            raise ValueError("expected_changes_per_model must be finite and at least 1")
        if preserve_feature_count and expected_changes_per_model != 1:
            raise ValueError(
                "expected_changes_per_model must equal 1 when preserve_feature_count is enabled"
            )
        if convergence_patience is not None:
            if isinstance(convergence_patience, (bool, np.bool_)) or not isinstance(
                convergence_patience, (int, np.integer)
            ):
        ...
        if isinstance(convergence_min_delta, (bool, np.bool_)) or not isinstance(
            convergence_min_delta, (int, float, np.integer, np.floating)
        ):
        ...
        if not np.isfinite(convergence_min_delta) or convergence_min_delta < 0:
            raise ValueError("convergence_min_delta must be finite and non-negative")
        ...
        if use_ensemble_validation and len(validation_indices) == 0:
            raise ValueError(
                "use_ensemble_validation requires a non-empty ensemble validation set in cvSet"
            )
        if not hasattr(self, "parameters_") or not self.models:
            raise RuntimeError("models must be fitted before stochastic feature selection")
        ...
        def mean_score():
            """Return the current mean member score."""
            if use_ensemble_validation:
                ...
                for model_index in range(self.num_models):
                    ...
                    try:
                        ...
                        scores.append(float(self.score(self, model_index=model_index)["score"]))
                    finally:
                        self.cv.test[model_index] = original_test
                return float(np.mean(scores))
            if self.separate_parameters:
                return float(np.mean([performance.score for performance in self.performance_]))
            return float(self.performance_.score)

        def snapshot():
            """Capture all mutable selection state for rollback."""
        ...
        def restore(state):
            """Restore a state captured before evaluating a proposal."""
            (
                self.models,
                self.kernel_matrix_,
                self.parameters_,
        ...
        def weighted_choices(ranks, prefer_high, count):
            """Sample rank-biased indices without replacement."""
            ...
            return np.atleast_1d(
                rng.choice(len(ranks), size=count, replace=False, p=weights / np.sum(weights))
            ).astype(int)

        def ranked_groups(operation, model_index):
            """Return feasible perturbation groups and their ranks."""
        ...
        def feasible_operations(model_index):
            """List add/remove operations allowed for one feature set."""
            operations = []
        ...
        def can_swap(model_index):
            """Return whether an equal-width add/remove swap exists."""
            ...
            return len(active) > 1 and any(
                ...
                for active_group in active
                for inactive_group in inactive
            )

        def propose_swap(model_index):
            """Propose one rank-guided equal-width feature-group swap."""
            ...
            removable_indices = [
                ...
                if any(len(group) == len(candidate) for candidate in inactive)
            ]
            selected_remove = int(
                weighted_choices(remove_ranks[removable_indices], prefer_high=False, count=1)[0]
            )
            remove_index = removable_indices[selected_remove]
            addable_indices = [
                index
                for index, group in enumerate(inactive)
                if len(group) == len(active[remove_index])
            ]
            selected_add = int(
                weighted_choices(add_ranks[addable_indices], prefer_high=True, count=1)[0]
            )
        ...
        def propose_moves(model_index):
            """Propose a stochastic batch of additions or removals."""
            ...
            requested_count = 1 + int(rng.poisson(expected_changes_per_model - 1.0))
            max_changes = len(groups) if operation == "add" else len(groups) - 1
            ...
            return [(operation, model_index, groups[group_index]) for group_index in group_indices]
        ...
        for iteration in range(int(n_iterations)):
            if preserve_feature_count:
                targets = range(self.num_models) if self.separate_feature_sets else [None]
                swap_batches = [propose_swap(model_index) for model_index in targets]
                moves = (
                    []
                    if any(not batch for batch in swap_batches)
                ...
                targets = list(range(self.num_models)) if self.separate_feature_sets else [None]
                eligible_targets = [
                    model_index for model_index in targets if feasible_operations(model_index)
                ]
                moves = (
                    []
                    if not eligible_targets
                    else propose_moves(eligible_targets[int(rng.integers(len(eligible_targets)))])
                )
            if not moves:
            ...
            for operation, model_index, group in moves:
                if operation == "add":
                    self._add_features(group, model_index=model_index, update_kernel=False)
                else:
                    self._remove_features(group, model_index=model_index, update_kernel=False)
            self._tune_member_models(parameter_grid)
            proposed_score = mean_score()
            score_change = proposed_score - current_score
            accepted = score_change >= 0 or rng.random() < np.exp(
                score_change / current_temperature
            )
            if accepted:
            ...
            changes_by_model = [
                ...
                for model_index in (
                    range(self.num_models) if self.separate_feature_sets else [None]
                )
            ]
            history.append(
                dotdict(
                    {
                        ...
                        "model_index": (model_indices[0] if len(moves) == 1 else model_indices),
                        "features_changed": (
                            changed_groups[0] if len(moves) == 1 else changed_groups
                        ),
                        "moves": [
                            dotdict(
                                {
                        ...
                        "objective": (
                            "ensemble_validation" if use_ensemble_validation else "cross_validation"
                        ),
                        "accepted": accepted,
            ...
            if (
                ...
                and iterations_without_improvement >= convergence_patience
            ):
                converged = True
    ...
    # ------------------------------------------------------------------
    # Ensemble-pooled stochastic refinement and diversity objectives
    # ------------------------------------------------------------------
    def ensemble_stochastic_feature_selection(
        ...
        ----------
        parameter_grid : iterable of mistic.utility.paramSet
            Parameter candidates used to retune each accepted proposal.
        n_iterations : int, default=100
            Maximum number of pooled proposals.
        temperature : float, default=0.05
            Initial simulated-annealing temperature.
        cooling_rate : float, default=0.97
            Multiplicative temperature decay per iteration.
        add_probability : float, default=0.5
            Probability of selecting additions when both directions exist.
        random_seed : int or None, default=None
            Seed controlling proposal sampling and acceptance.
        convergence_patience : int or None, default=20
            Non-improving proposals allowed before early stopping.
        convergence_min_delta : float, default=0.0
            Minimum objective gain counted as improvement.
        preserve_feature_count : bool, default=False
            Restrict proposals to equal-sized feature-group swaps.
        feature_diversity_weight : float, default=0.0
            Reward assigned to mean pairwise feature-set distance.
        prediction_diversity_weight : float, default=0.0
            Reward assigned to prediction decorrelation.
        performance_tolerance : float or None, default=None
            Maximum score loss allowed relative to the best observed score.
        max_feature_similarity : float or None, default=None
            Maximum permitted pairwise feature-set Jaccard similarity.
        ...
        if performance_tolerance is not None:
            if not np.isfinite(performance_tolerance) or performance_tolerance < 0:
        ...
        if max_feature_similarity is not None:
            if not np.isfinite(max_feature_similarity) or not 0 <= max_feature_similarity <= 1:
        ...
        if not self.separate_feature_sets:
            raise ValueError("ensemble pooled refinement requires separate_feature_sets=True")

        rng = np.random.default_rng(random_seed)
        score_weight = float(getattr(getattr(self.score, "__self__", None), "weight", 0.5))

        def score_predictions(indices, predictions):
            """Score aggregate predictions on the requested sample indices."""
            ...
            if isinstance(self.SVM, SVR):
                ...
                return float(score_weight * pearson + (1 - score_weight) * max(0.00001, r2))

            classes = np.asarray(self.models[0].classes_)
            if classes.size != 2:
                raise ValueError("ensemble pooled refinement requires binary classification")
        ...
        def out_of_fold_state():
            """Collect member outputs and the pooled out-of-fold score."""
            ...
            for model_index in range(self.num_models):
                ...
                if isinstance(self.SVM, SVR):
                    ...
                    values = self.decision_function(self.cv.X[indices], model_index=model_index)
                outputs.append(np.asarray(values, dtype=float))
                sums[indices] += values
                counts[indices] += 1
            covered = np.flatnonzero(counts > 0)
            if covered.size == 0:
                raise ValueError("out-of-fold ensemble scoring requires non-empty CV test sets")
            aggregate = sums[covered] / counts[covered]
            return sums, counts, covered, aggregate, outputs

        def feature_diversity(feature_sets):
            """Return mean pairwise Jaccard distance between feature sets."""
            distances = []
            similarities = []
            normalized = [set(np.asarray(features).tolist()) for features in feature_sets]
            for first in range(len(normalized)):
                for second in range(first + 1, len(normalized)):
                    union = normalized[first] | normalized[second]
                    similarity = (
                        len(normalized[first] & normalized[second]) / len(union) if union else 1.0
                    )
        ...
        def prediction_diversity(outputs):
            """Return decorrelation-based diversity across member outputs."""
            distances = []
            for first in range(self.num_models):
                ...
                for second in range(first + 1, self.num_models):
                    ...
                    if first_std == 0 or second_std == 0:
                        correlation = 1.0 if np.allclose(first_values, second_values) else 0.0
                    else:
                        correlation = np.corrcoef(first_values, second_values)[0, 1]
                    distances.append(1.0 - abs(float(correlation)))
            return float(np.mean(distances)) if distances else 0.0

        def combined_objective(score, feature_distance, prediction_distance):
            """Combine predictive score with configured diversity rewards."""
            return float(
                score
        ...
        def candidate_feature_sets(candidate):
            """Return feature sets after applying a candidate move."""
            proposed = [np.asarray(features, dtype=int).copy() for features in self.features]
        ...
        def snapshot():
            """Capture mutable ensemble state for proposal rollback."""
            return copy.deepcopy(
                (
                    self.models,
                    self.kernel_matrix_,
        ...
        def restore(state):
            """Restore ensemble state after a rejected proposal."""
            (
                self.models,
                self.kernel_matrix_,
                self.parameters_,
        ...
        def add_diversity(candidate, proposed_outputs):
            """Attach diversity metrics and objective to a candidate."""
            candidate_features = candidate_feature_sets(candidate)
            feature_distance, maximum_similarity = feature_diversity(candidate_features)
            ...
            candidate.estimated_objective = combined_objective(
                candidate.estimated_score, feature_distance, prediction_distance
            )
            if (
                max_feature_similarity is not None
                and maximum_similarity > max_feature_similarity
                and maximum_similarity >= current_max_similarity
            ):
                return False
            return not (
                performance_tolerance is not None
                and candidate.estimated_score < peak_score - performance_tolerance
            )

        def candidate_pool(sums, counts, covered, outputs):
            """Enumerate and estimate all feasible pooled feature moves."""
            candidates = []
            previous_direction = getattr(self, "_selection_direction_", None)
            try:
                if preserve_feature_count:
                    for model_index in range(self.num_models):
                        ...
                        self._selection_direction_ = "forward"
                        add_effects = self.decision_perturbation_(model_index, self.cv.X[indices])
                        for remove_index, remove_group in enumerate(active):
                            for add_index, add_group in enumerate(inactive):
                                ...
                                proposed_sums[indices] -= (
                                    remove_effects[:, remove_index] + add_effects[:, add_index]
                                )
                                ...
                                candidate = dotdict(
                                    {
                                        ...
                                        "estimated_score": score_predictions(covered, proposed),
                                    }
                                )
                ...
                for operation in ("add", "remove"):
                    self._selection_direction_ = "forward" if operation == "add" else "backward"
                    for model_index in range(self.num_models):
                        groups = (
                            self._inactive_perturbation_sets(model_index)
                            if operation == "add"
                            else self._active_perturbation_sets(model_index)
                        )
                        ...
                        if not groups:
                            continue
                        indices = np.asarray(self.cv.test[model_index], dtype=int)
                        perturbations = self.decision_perturbation_(model_index, self.cv.X[indices])
                        for group_index, group in enumerate(groups):
                            ...
                            proposed_outputs = list(outputs)
                            proposed_outputs[model_index] = (
                                outputs[model_index] - perturbations[:, group_index]
                            )
                            if add_diversity(candidate, proposed_outputs):
                                candidates.append(candidate)
            finally:
        ...
        current_feature_diversity, current_max_similarity = feature_diversity(self.features)
        current_prediction_diversity = prediction_diversity(outputs)
        current_objective = combined_objective(
            current_score, current_feature_diversity, current_prediction_diversity
        )
        current_state = snapshot()
        best_score = current_score
        ...
        for iteration in range(int(n_iterations)):
            ...
            if preserve_feature_count:
                ...
                operation_pool = [
                    ...
                    if candidate.operation == ("add" if choose_add else "remove")
                ]
                if not operation_pool:
                    operation_pool = candidates
            ranks = rank_items(
                np.asarray([candidate.estimated_objective for candidate in operation_pool])
            )
            weights = ranks.astype(float) + 1.0
            selected = operation_pool[
                int(rng.choice(len(operation_pool), p=weights / np.sum(weights)))
            ]

            if selected.operation == "swap":
                self._remove_features(
                    selected.features_removed, model_index=selected.model_index, update_kernel=False
                )
                self._add_features(
                    selected.features_added, model_index=selected.model_index, update_kernel=False
                )
            elif selected.operation == "add":
                self._add_features(
                    selected.features_changed, model_index=selected.model_index, update_kernel=False
                )
            else:
                self._remove_features(
                    selected.features_changed, model_index=selected.model_index, update_kernel=False
            ...
            proposed_score = score_predictions(proposed_covered, proposed_aggregate)
            proposed_feature_diversity, proposed_max_similarity = feature_diversity(self.features)
            proposed_prediction_diversity = prediction_diversity(proposed_outputs)
            proposed_objective = combined_objective(
                proposed_score, proposed_feature_diversity, proposed_prediction_diversity
            )
            objective_change = proposed_objective - current_objective
            meets_performance = (
                performance_tolerance is None
                or proposed_score >= peak_score - performance_tolerance
            )
            meets_similarity = (
                max_feature_similarity is None
                or proposed_max_similarity <= max_feature_similarity
                or proposed_max_similarity < current_max_similarity
            )
            accepted = bool(
                meets_performance
                and meets_similarity
                and (
                    objective_change >= 0
                    or rng.random() < np.exp(objective_change / current_temperature)
                )
            )
            if accepted:
                current_score = proposed_score
                ...
                best_outside_tolerance = (
                    performance_tolerance is not None
                    and best_score < peak_score - performance_tolerance
                )
                if proposed_objective > best_objective or best_outside_tolerance:
                    best_score = proposed_score
                    ...
                    best_state = snapshot()
                if proposed_objective > convergence_objective + convergence_min_delta:
                ...
                dotdict(
                    {
                        ...
                        "features_removed": copy.deepcopy(
                            getattr(selected, "features_removed", None)
                        ),
                        "features_added": copy.deepcopy(getattr(selected, "features_added", None)),
                        "estimated_score": selected.estimated_score,
                        "estimated_objective": selected.estimated_objective,
                        "estimated_feature_diversity": selected.estimated_feature_diversity,
                        "estimated_prediction_diversity": selected.estimated_prediction_diversity,
                        "score": proposed_score,
            ...
            current_temperature *= cooling_rate
            if convergence_patience is not None and stale_iterations >= convergence_patience:
                converged = True
                stop_reason = "converged"
                break
        ...
        self.ensemble_stochastic_best_feature_diversity_ = best_feature_diversity
        self.ensemble_stochastic_best_prediction_diversity_ = best_prediction_diversity
    ...
    def set_num_features(self, num_features, parameter_grid):
        ...
        if isinstance(num_features, (bool, np.bool_)) or not isinstance(
            num_features, (int, np.integer)
        ):
            raise TypeError("num_features must be an integer")
        if num_features < 1:
            raise ValueError("num_features must be at least 1")

        rankings = self.sorted_features if self.separate_feature_sets else [self.sorted_features]
        ...
        if any(num_features > len(ranking) for ranking in rankings):
            raise ValueError("num_features cannot exceed the number of ranked features")

        if self.separate_feature_sets:
            for model_index, ranking in enumerate(rankings):
                self._set_features(
                    np.asarray(ranking)[:num_features], model_index=model_index, update_kernel=False
                )
        else:
            self._set_features(np.asarray(rankings[0])[:num_features], update_kernel=False)
    ...
    def greedy_backward_selection(
        ...
        reduction_factor : float, default=0.1
            Fraction of active groups removed per iteration; zero removes one.
        feature_ranker : callable
            Callable returning removal ranks for a fitted member.
        set_for_rank : str, default="train"
            CV index collection used by ``feature_ranker``.
        tune_models_each_step : bool, default=True
            Whether to run full parameter tuning after every removal.
        ...
        def fit_current_feature_set():
            """Tune and summarize the currently active feature set."""
            ...
            if isinstance(scaled_parameters, list):
                ...
                if "gamma" in scaled_parameters.kernel:
                    scaled_parameters.kernel["gamma"] *= np.mean(initial_feature_counts) / np.mean(
        ...
        while n_sets >= 2:
            ...
            if self.separate_parameters:
                ...
                for m in range(1, self.num_models):
                    mean_performance = {
                        key: mean_performance[key] + self.performance_[m][key]
                        for key in self.performance_[m]
                    }
                row = dotdict(
                    {key: mean_performance[key] / self.num_models for key in mean_performance}
                )
            else:
            ...
            if self.separate_feature_sets:
                row["num_features"] = (
                    np.sum([len(self.features[m]) for m in range(self.num_models)])
                    / self.num_models
                )
            else:
            ...
            row["mean_nSV"] = (
                np.sum(np.sum([self.models[m].n_support_ for m in range(self.num_models)]))
                / self.num_models
            )
            ...
            if self.separate_feature_sets:
                for i in range(self.num_models):
                    ...
                    n_to_remove = max(
                        1,
                        min(
                            len(active_sets) - 1, int(np.floor(len(active_sets) * reduction_factor))
                        ),
                    )
                    selected_sets = np.argsort(feature_rank)[:n_to_remove]
                    to_remove = np.concatenate([active_sets[index] for index in selected_sets])
                    self._remove_features(to_remove, model_index=i, update_kernel=False)
                ...
                n_to_remove = max(
                    1, min(len(active_sets) - 1, int(np.floor(len(active_sets) * reduction_factor)))
                )
    ...
    def greedy_forward_selection(
        ...
        medoids = set(getattr(self.cv, "feature_medoids_", np.arange(self.cv.X.shape[1])).tolist())
        candidates = [
            candidate for candidate in candidates if medoids.intersection(candidate.tolist())
        ]
        ...
        if max_features is None:
            ...
            raise ValueError(f"max_features must be between 1 and {total_features}")

        eligible_candidates = [
            candidate for candidate in candidates if len(candidate) <= max_features
        ]
        ...
        try:
            ...
            for candidate_index, candidate in enumerate(eligible_candidates):
                if self.separate_feature_sets:
                    for model_index in range(self.num_models):
                        self._set_features(candidate, model_index, update_kernel=False)
                else:
            ...
            def fit_current_feature_set():
                """Tune and summarize the current forward feature set."""
            ...
            while True:
                ...
                row["mean_nSV"] = (
                    np.sum([np.sum(model.n_support_) for model in self.models]) / self.num_models
                )
                print(f"Number of Features: {row['num_features']:.0f}, Score: {row['score']:.3f}")
                ...
                if self.separate_feature_sets:
                    ...
                    for model_index in range(self.num_models):
                        ...
                        self._add_features(additions, model_index, update_kernel=False)
                        added_any = True
                    if not added_any:
                        break
                else:
            ...
            if max_features < total_features:
                if self.separate_feature_sets:
                    for model_index in range(self.num_models):
                        self._set_features(
                            np.arange(total_features), model_index, update_kernel=False
                        )
                else:
                    self._set_features(np.arange(total_features), update_kernel=False)
                fit_current_feature_set()
                row = mean_row()
            ...
            if self.separate_feature_sets:
                ...
                for model_index, selected in enumerate(selection_order):
                    unselected = [
                        feature for feature in range(total_features) if feature not in selected
                    ]
                ...
                unselected = [
                    feature for feature in range(total_features) if feature not in selection_order
                ]
    ...
    def _normalize_perturbation_sets(perturbation_sets, available_features):
        ...
        Parameters
        ----------
        perturbation_sets : sequence of sequences of int
            Proposed non-overlapping feature groups.
        available_features : array-like of int
            Complete collection of valid feature indices.

        Returns
        -------
        list of list
            Deduplicated groups covering every available feature exactly once.
        """
        ...
        for group in perturbation_sets:
            ...
            if unknown_features:
                raise ValueError(
                    f"perturbation set contains unknown features: {sorted(unknown_features)}"
                )
        ...
        flattened_features = [feature for group in normalized_sets for feature in group]
        ...
        if missing_features:
            raise ValueError(
                f"perturbation_sets must include every feature; missing: {sorted(missing_features)}"
            )
        return normalized_sets

    def _active_perturbation_sets(self, model_index=None):
        """Return perturbation groups represented in the active features.

        Parameters
        ----------
        model_index : int or None, default=None
            Member index required for separate feature sets.

        Returns
        -------
        list of numpy.ndarray
            Active members of every represented perturbation group.
        """
        if self.separate_feature_sets:
            if model_index is None:
        ...
        return [
            np.asarray([feature for feature in perturbation_set if feature in active_features])
            for perturbation_set in self.perturbation_sets
    ...
    def _inactive_perturbation_sets(self, model_index=None):
        ...
        -------
        list of numpy.ndarray
            Perturbation groups having no active feature in the model.
        """
        if self.separate_feature_sets:
            if model_index is None:
                raise ValueError("model_index is required for separate feature sets")
    ...
    def feature_importance_(self, model_index):
        ...
        -------
        numpy.ndarray
            Frozen-objective criterion for each active group, or each inactive
            group during forward selection.
        ...
        K = self.kernel.compute(
            support_vectors, feature_index=current_features, parameters=parameters
        )
        ...
        for z, perturbation_set in enumerate(normalized_sets):
            if forward:
                ...
                features_z = current_features[~np.isin(current_features, perturbation_set)]
            Kp = self.kernel.compute(
                support_vectors, feature_index=features_z, parameters=parameters
            )
    ...
    def probability_perturbation_(self, model_index, X):
        ...
        ----------
        model_index : int
            Index of the fitted binary SVC member.
        X : numpy.ndarray of shape (n_samples, n_features)
            Observations at which effects are evaluated.

        Returns
        -------
        numpy.ndarray
            Approximate probability change for each sample and perturbation
            group.
        """
    ...
    def decision_perturbation_(self, model_index, X):
        ...
        model_index : int
            Index of the fitted ensemble member.
        X : numpy.ndarray of shape (n_samples, n_features)
            Observations at which perturbations are evaluated.

        Returns
        -------
        numpy.ndarray
            Decision change for each sample and active perturbation group, or
            inactive group during forward selection.
        """
        support_vectors = self._get_support_vectors(model_index)
        ...
        K = self.kernel.compute(
            X=support_vectors, feature_index=current_features, parameters=parameters, Y=X
        )
        ...
        for z, perturbation_set in enumerate(perturbation_sets):
            ...
            if not forward and len(features_z) == 0:
                raise ValueError("a perturbation set cannot remove all active features")
            Kp = self.kernel.compute(
                support_vectors, feature_index=features_z, parameters=parameters, Y=X
            )
    ...
    def decision_gradient_(self, model_index, X):
        ...
        model_index : int
            Index of the fitted ensemble member.
        X : numpy.ndarray of shape (n_samples, n_features)
            Observations at which gradients are evaluated.
    ...
    def probability_gradient_(self, model_index, X):
        ...
        ----------
        model_index : int
            Index of a fitted probability-enabled binary SVC member.
        X : numpy.ndarray of shape (n_samples, n_features)
            Observations at which gradients are evaluated.
        ...
        -------
        numpy.ndarray
            Positive-class probability gradient for each sample and active
            feature.
        """
        model = self.models[model_index]
        if self._is_one_class() or isinstance(self.SVM, SVR):
        ...
        probability = model.predict_proba(self._inference_kernel(X, model_index))[:, 1]
        slope = -float(np.ravel(model.probA_)[0]) * probability * (1 - probability)
    ...
    def _find_boundary_points(self, model_index, X):
        ...
        model_index : int
            Index of the fitted ensemble member.
        X : numpy.ndarray of shape (n_samples, n_features)
            Starting observations for boundary optimization.
    ...
    def integrated_gradient(
        ...
        model_index : int or None, default=None
            Member to explain, or ``None`` to average all members.
        num_steps : int, default=20
            Number of points sampled along each integration path.
        reference_point : array-like or None, default=None
            Shared baseline vector or one baseline per observation.
        ref_point : array-like or None, default=None
        ...
        -------
        numpy.ndarray
            Attribution matrix with one row per sample and one column per
            selected feature.
        """
        ...
        if output == "probability":
            ...
            if np.asarray(model.classes_).size != 2:
                raise ValueError("probability integrated gradients currently require binary SVC")
        ...
        if supplied_reference:
            ...
            if reference_point.ndim == 1:
                ...
                raise ValueError("reference_point must have shape (n_features,) or match X")
        ...
        for m in model_indices:
            ...
            for i in range(len(X)):
                ...
                gradient_steps = (
                    self.probability_gradient_(m, x_steps)
                    if output == "probability"
                    else self.decision_gradient_(m, x_steps)
                )
    ...
    def explain_integrated_gradients(
        ...
        feature_names : sequence of str or None, default=None
            Names for all input columns or only selected columns.
        target : array-like or None, default=None
            Optional sample labels or targets stored for visualization.
        model_index : int or None, default=None
            Member to explain, or ``None`` to average all members.
        num_steps : int, default=20
            Number of points sampled along each integration path.
    ...
    def find_knee(self, metric="score"):
        ...
        try:
            ...
            raise KeyError(f"feature performance does not contain {error.args[0]!r}") from error
        ...
        performance = np.asarray(
            [np.max(points[points[:, 0] == count, 1]) for count in feature_counts]
        )
        ...
        normalized_features = (feature_counts - feature_counts[0]) / feature_range
        normalized_performance = (performance - np.min(performance)) / performance_range
    ...
    def predict(self, X, model_index=None, use_voting=False, prediction_mode="unified"):
        ...
        model_index : int or None, default=None
            Specific member to use, or ``None`` for aggregate prediction.
        use_voting : bool, default=False
            Use majority voting rather than averaged decision values for
            classification member sets.
        prediction_mode : {"unified", "set"}, default="unified"
            Select the final unified estimator or cross-validation members.
        ...
        if self._is_one_class():
            ...
            if use_voting:
                ...
                for m in model_indices:
                    votes += self.models[m].predict(self._inference_kernel(X, m))
                predictions = np.where(votes >= 0, 1, -1)
            else:
                decision_values = self.decision_function(X, model_index, prediction_mode="set")
            ...
            if use_voting:
                ...
                predictions = self.models[0].classes_[
                    (prediction_counts / len(model_indices) < 0.5) + 0
                ]

            else:
                decision_values = self.decision_function(X, model_index, prediction_mode="set")
                cutoff = (
                    getattr(self, "decision_value_cutoff_", 0.0) if model_index is None else 0.0
                )
                predictions = self.models[0].classes_[(decision_values > cutoff) + 0]
    ...
    def predict_proba(self, X, model_index=None, prediction_mode="unified"):
        ...
        ----------
        X : numpy.ndarray of shape (n_samples, n_features)
            Observations for which probabilities are requested.
        model_index : int or None, default=None
            Specific member to use, or ``None`` for aggregate prediction.
        prediction_mode : {"unified", "set"}, default="unified"
            Select the final unified estimator or cross-validation members.
        ...
        if model_index is None and prediction_mode == "unified":
            if self.unified_model_ is not None:
                return self.unified_model_.predict_proba(self._unified_inference_kernel(X))
            prediction_mode = "set"

        model_indices = range(self.num_models) if model_index is None else [model_index]
        ...
        for m in model_indices:
            ...
            positions = [int(np.flatnonzero(classes == label)[0]) for label in model.classes_]
    ...
    def _set_decision_function(self, X, model_index=None):
        """Return the ensemble-member decision function for calibration.
        ...
        model_index : int or None, default=None
            Specific member to use, or ``None`` to average all members.

        Returns
        -------
        numpy.ndarray
            Member or mean ensemble decision values.
        """
        if model_index == None:
            model_indices = [i for i in range(self.num_models)]
        else:
    ...
    def decision_function(self, X, model_index=None, prediction_mode="unified"):
        ...
        X : numpy.ndarray of shape (n_samples, n_features)
            Observations to evaluate.
        model_index : int or None, default=None
            Specific member to use, or ``None`` for aggregate inference.
        prediction_mode : {"unified", "set"}, default="unified"
            Select the final unified estimator or cross-validation members.
        ...
        if prediction_mode not in {"unified", "set"}:
            raise ValueError("prediction_mode must be 'unified' or 'set'")
        if model_index is None and prediction_mode == "unified" and self.unified_model_ is not None:
            return self.unified_model_.decision_function(self._unified_inference_kernel(X))
    ...
    def enrichment_score(self, metric="score", type="auc"):
        ...
        ----------
        metric : str, default="score"
            Performance key read from each feature-selection result.
        type : {"auc", "max"}, default="auc"
            Summary statistic: normalized curve area or maximum value.
