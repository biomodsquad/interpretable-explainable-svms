...
class IntegratedGradientsResult:
    ...
    def summary_plot(self, ax=None, max_features=None, jitter=0.22,
                     cmap="coolwarm", random_state=0, scatter_kwargs=None):
        """Draw an attribution summary (beeswarm-style) plot."""
        if ax is None:
            _, ax = plt.subplots()
        count = self.values.shape[1] if max_features is None else min(max_features, self.values.shape[1])
        ...
        for row, feature in enumerate(order):
            artist = ax.scatter(
                self.values[:, feature], row + rng.uniform(-jitter, jitter, len(self.values)),
                c=self.inputs[:, feature], cmap=cmap, **options)
    ...
    def heatmap(self, ax=None, target=None, cmap="coolwarm", center=0.0,
                cluster=False, imshow_kwargs=None, target_cmap=None,
                attribution_colorbar_kwargs=None, target_colorbar_kwargs=None,
                target_strip_width=0.10, strip_pad=0.04,
                colorbar_width=0.16, colorbar_pad=0.08,
                colorbar_gap=0.06, dendrogram_width=0.75,
                dendrogram_pad=0.04, dendrogram_linewidth=0.8,
        ...
        ax.set_xticks(range(len(column_order)),
                      [self.feature_names[i] for i in column_order], rotation=90)
        ...
        ax.figure.colorbar(
            image, cax=attribution_colorbar_ax, **attribution_bar_options)
        ...
        if annotation is not None:
            ...
            if categorical:
                ...
                norm = BoundaryNorm(np.arange(len(unique) + 1) - .5, len(unique))
                target_image = strip.imshow(encoded[:, np.newaxis], aspect="auto",
                                            cmap=chosen_cmap, norm=norm)
                target_bar_options = {
                ...
                bar = ax.figure.colorbar(
                    target_image, cax=target_colorbar_ax, **target_bar_options)
                bar.ax.set_yticklabels([str(value) for value in unique])
            else:
                target_image = strip.imshow(ordered[:, np.newaxis], aspect="auto",
                                            cmap=target_cmap or "viridis")
                target_bar_options = {
                    "label": "Target",
                    **(target_colorbar_kwargs or {}),
                }
    ...
    def interaction_plot(self, feature=None, interaction_feature=None, ax=None,
                         cmap="viridis", scatter_kwargs=None):
        ...
        artist = ax.scatter(self.inputs[:, first], self.values[:, first],
                            c=self.inputs[:, second], cmap=cmap, **options)
